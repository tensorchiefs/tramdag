...
| Name | Role |
|----------------------------------|------------------------------------------------------------------------------|
| [`Term`][tramdag.spec.Term] | One additive term of a node's transformation, frozen data; each term is a subclass whose class name is its `name` and whose annotated attributes are its options (defaults dropped on serialization, so equality is canonical). `+` on terms builds plain lists. Carries the spec-level rules: `edge_parents`, `cells`, `classical`, `options()`, `from_serialized`. Subclass `Term` for a new term; its module goes in `terms.py` and read as attributes (`term.penalty`, `term.units`, ...). |
